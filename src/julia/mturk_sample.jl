...
## NOW MERGE THE RESULTS WITH THE GCS SCORES: 
# retrain the model here - using code from transfer_analyses.jl resnet model 
include("train_base_resnet_svm.jl");
...
mturk = CSV.read("data/mturk_1k_training.csv", DataFrame)
rename!(myfn, mturk)
...
l1 = Gadfly.layer(out, x=:turk_gcs, y=:probs, Geom.point, alpha=[.5], Theme(highlight_width=0mm));
l2 = layer(out, x=:turk_gcs, y=:probs, color=[colorant"lightsalmon"], Geom.smooth, Theme(line_width=2mm));
mturk_plot = Gadfly.plot(l1, l2, Guide.xlabel("Human Annotated Expression Scores"), Guide.ylabel("GCS"), Guide.title("GCS vs Annotated Images"))
...
## NEED the MTURK results PLUS the 
